...
import serviceRequestService, { ServiceRequest } from '../../api/serviceRequestService';
...
const ServiceRequestForm = () => {
      ...
      const fetchServiceRequest = async () => {
        try {
          ...
          setFormData({
            ...
            scheduledDate: serviceReq.scheduledDate ? new Date(serviceReq.scheduledDate) : null,
            completionDate: serviceReq.completionDate ? new Date(serviceReq.completionDate) : null
          });
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        // Fetch customers
        const customerResponse = await customerService.getCustomers({ limit: 100 });
        setCustomers(customerResponse.data.customers);
        
        // Fetch all projects for project selection
        const projectResponse = await projectService.getProjects({ limit: 100 });
        setProjects(projectResponse.data.projects);
        
        // Fetch technicians (this would be a users API with role filter in a real app)
        // For now, we'll mock a few technicians
        setTechnicians([
          { _id: 'tech1', firstName: 'John', lastName: 'Smith', email: 'john@solar.com' },
          { _id: 'tech2', firstName: 'Sarah', lastName: 'Johnson', email: 'sarah@solar.com' },
          { _id: 'tech3', firstName: 'Michael', lastName: 'Davis', email: 'michael@solar.com' }
        ]);
      } catch (err: any) {
  ...
  const fetchCustomerProjects = async (customerId: string) => {
    try {
      const response = await projectService.getProjects({ customer: customerId });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedData = {
        ...formData,
        scheduledDate: formData.scheduledDate ? formatDate(formData.scheduledDate) : undefined,
        completionDate: formData.completionDate ? formatDate(formData.completionDate) : undefined
      };
  ...
  const handleCompleteRequest = async () => {
    ...
    try {
      ...
      const response = await serviceRequestService.completeServiceRequest(id, completionDetails);
    ...
    <Box>
      ...
      <form onSubmit={handleSubmit}>
                  ...
                  <Grid item xs={12}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        onChange={handleSelectChange}
                        disabled={!formData.customer || formData.status === 'completed' || formData.status === 'cancelled'}
                      >
                  ...
                  {isEditing && formData.status === 'completed' && (
                    <Grid item xs={12}>
                      <TextField
                        ...
                        type="date"
                        value={formData.completionDate ? formatDate(formData.completionDate) : ''}
                        onChange={(e) => {
                          const date = e.target.value ? new Date(e.target.value) : null;
        ...
        <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end' }}>
          ...
          <Button 
            ...
            variant="contained" 
            startIcon={<SaveIcon />}
            disabled={loading || formData.status === 'completed' || formData.status === 'cancelled'}
          >
      ...
      <Dialog open={noteDialogOpen} onClose={() => setNoteDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddNote} variant="contained" disabled={!noteText.trim() || loading}>
      ...
      <Dialog open={completeDialogOpen} onClose={() => setCompleteDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography paragraph sx={{ mt: 2 }}>
            Are you sure you want to mark this service request as completed? This will set the status to "Completed" and record today's date as the completion date.
          </Typography>
          <TextField
            margin="dense"
        ...
        <DialogActions>
          ...
          <Button onClick={handleCompleteRequest} variant="contained" color="success" disabled={loading}>
