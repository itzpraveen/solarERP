      ...
      try {
        ...
        if (!dashboardData) {
          ...
          if (Array.isArray(projectsData) && projectsData.length > 0) {
            ...
            const projectsWithSize = projectsData.filter((project: any) =>
              project && project.systemSize && !isNaN(parseFloat(project.systemSize))
            );
            
            if (projectsWithSize.length > 0) {
              const totalSize = projectsWithSize.reduce((sum: number, project: any) =>
                sum + parseFloat(project.systemSize), 0
              );
              avgSystemSize = parseFloat((totalSize / projectsWithSize.length).toFixed(1));
            }
            
            // Calculate average project value
            const projectsWithValue = projectsData.filter((project: any) =>
              project && project.financials && project.financials.totalContractValue &&
              !isNaN(parseFloat(project.financials.totalContractValue))
            );
            
            if (projectsWithValue.length > 0) {
              const totalValue = projectsWithValue.reduce((sum: number, project: any) =>
                sum + parseFloat(project.financials.totalContractValue), 0
              );
              avgProjectValue = Math.round(totalValue / projectsWithValue.length);
            }
            
            // Calculate average completion days for completed projects
            const completedProjects = projectsData.filter((project: any) =>
              project && project.status === 'completed' &&
              project.dates && project.dates.projectClosed && project.createdAt
            );
