...
const Customers = () => {
  ...
  const handleCreateCustomer = async (customerData: any) => {
    try {
      ...
      // Use standard customer creation API for both new customers and conversions
      // The originalLead field in the data will maintain the reference
      const response = await customerService.createCustomer(customerData);
      console.log('Customer created successfully:', response);
      
      // If this was a lead conversion, we should update the lead status to "won"
      if (customerData.originalLead) {
        try {
          await leadService.updateStatus(customerData.originalLead, 'won');
          console.log('Lead status updated to won after conversion');
        } catch (updateErr) {
          console.warn('Could not update lead status after conversion:', updateErr);
          // Continue regardless of status update success
        }
      }
