      ...
      expenses: [
        {
          category: {
            ...
            enum: ['equipment', 'labor', 'permits', 'subcontractor', 'other'],
          },
          description: {
            type: String,
...
projectSchema.pre(/^find/, function (next) {
  ...
  if (currentQuery.active === undefined) {
    ...
    console.log(
      'Project find middleware - Adding active filter: { active: { $ne: false } }'
    );
  } else {
    console.log(
      'Project find middleware - Active filter already present:',
      currentQuery.active
    );
  }
  next();
  ...
  if (this.isModified('stage')) {
    ...
    switch (this.stage) {
      ...
      case 'scheduled':
        ...
        break;
      case 'in_progress':
        if (!this.dates.installationStarted)
          this.dates.installationStarted = now;
        break;
      case 'inspection':
        if (!this.dates.installationCompleted)
          this.dates.installationCompleted = now;
        break;
      case 'completed':
        if (!this.dates.inspectionCompleted)
          this.dates.inspectionCompleted = now;
