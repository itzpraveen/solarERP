...
if (process.env.NODE_ENV === 'production') {
  try {
    ...
    if (!fs.existsSync(buildPath)) {
      console.log('Build directory not found. This could cause "Cannot GET /" errors.');
      console.log('The frontend build may be missing. Check your deployment configuration.');
    } 
    // Check if index.html exists
    else if (!fs.existsSync(indexHtmlPath)) {
      console.warn('Build directory exists but index.html not found. Frontend may not be properly built.');
      console.log('Static files may be missing, which could cause "Cannot GET /" errors.');
    }
