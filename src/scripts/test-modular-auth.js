...
const { AuthenticationError, NotFoundError } = require('../common/utils/errors');
...
async function testLoginWithIncorrectCredentials() {
  try {
    ...
    if (error instanceof AuthenticationError) {
      console.log('Login correctly failed with incorrect credentials:', error.message);
      return;
    }
    throw error;
  }
...
async function testGetUserByNonExistentId() {
  try {
    ...
    if (error instanceof NotFoundError) {
      console.log('Get user by non-existent ID correctly failed:', error.message);
      return;
    }
    throw error;
  }
...
async function cleanUp() {
  try {
    // Find the test user model directly to bypass repository
